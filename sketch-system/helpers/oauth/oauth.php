...
function get_curl_response($toHeader, $url, $post = true,$data='',$isXML='') {
	$ch = curl_init();
...
function init($token_secret='',$oauth_token='',$oauth_verifier='') {
	$this->_public = false;
	$this->signatureMethod = new OAuthSignatureMethod_HMAC_SHA1();
	$this->consumer = new OAuthConsumer('wd7neeteaq1a', 'LLwPwojpiV0T8jC5', NULL);
	$this->token = $oauth_token;
	$this->secret = $token_secret;
	$this->oauth_verifier = $oauth_verifier;
}
function get_request_token() {
	$reqObj = OAuthRequest::from_consumer_and_token($this->consumer, NULL, "POST", 'https://api.linkedin.com'. '/uas/oauth/requestToken');
	$reqObj->set_parameter("oauth_callback",urlPath(sketch("menu_guid"))."?_api=accesstoken"); # part of OAuth 1.0a - callback now in requestToken
	$reqObj->sign_request($this->signatureMethod, $this->consumer, NULL);
	$toHeader = $reqObj->to_header();
	$output = $this->get_curl_response($toHeader, 'https://api.linkedin.com'. '/uas/oauth/requestToken');
	parse_str($output, $oauth);
	return array('url'=>'https://api.linkedin.com' . '/uas/oauth/authorize' . '?oauth_token=' . $oauth['oauth_token'],'token'=>$oauth['oauth_token'],'secret'=>$oauth['oauth_token_secret']);
}

function get_access_token() {
	$token = new OAuthConsumer($this->token, $this->secret, 1);
	$accObj = OAuthRequest::from_consumer_and_token($this->consumer, $token, "POST", 'https://api.linkedin.com'.'/uas/oauth/accessToken');
	$accObj->set_parameter("oauth_verifier", $this->oauth_verifier); # need the verifier too!
	$accObj->sign_request($this->signatureMethod, $this->consumer, $token);
	$toHeader = $accObj->to_header();
	$output = $this->get_curl_response($toHeader, 'https://api.linkedin.com'.'/uas/oauth/accessToken');
	parse_str($output, $oauth);
	return array('token'=>$oauth['oauth_token'],'secret'=>$oauth['oauth_token_secret']);
}

function post_status($comment,$title='',$url='',$image='',$visible='anyone',$requestURL="http://api.linkedin.com/v1/people/~/shares"){	
	$data = '<?xml version="1.0" encoding="UTF-8"?>
	<share>
	  <comment>'.trim(strip_tags($comment)).'</comment>';
	  if(trim($title) != ''){ 
		$data .= '<content>
		 <title>'.$title.'</title>';
		 if(trim($url)!=''){
			$data .= '<submitted-url>'.$url.'</submitted-url>';
		 }
		 if($image .= ''){
			$data .= '<submitted-image-url>'.urlPath($image).'</submitted-image-url>';
		}
		$data .='</content>';
	  }
	  $data .= '<visibility>
		 <code>'.trim(strip_tags($visible)).'</code>
	  </visibility>
	</share>';
	$token 		= new OAuthConsumer($this->token, $this->secret, 1);
    $profileObj = OAuthRequest::from_consumer_and_token($this->consumer, $token, "POST", $requestURL);
	$profileObj->sign_request($this->signatureMethod, $this->consumer, $token);
	$toHeader 	= $profileObj->to_header();
	return $this->get_curl_response($toHeader, $requestURL,true,$data,true);
}

function get_profile($requestURL) {
	$endpoint = $requestURL.":(".join(',',array(	'id',
						'first-name',
						'last-name',
						'headline',
						'picture-url')).")";
...
$this->get_profile('https://api.linkedin.com' . '/v1/people/~');
}
...
$this->get_profile('https://api.linkedin.com' . '/v1/people/url=' . urlencode($publicURL));
}

function get_public_profile_by_member_token($memberToken) {
$this->get_profile('https://api.linkedin.com' . '/v1/people/id=' . $memberToken);
}
